...
data CliState = CliState { mOpts :: MainOpts }

data BudgetExistsException = BudgetExistsException
    deriving Typeable

instance Show BudgetExistsException where
    show BudgetExistsException =
        "budget.yaml already exists!"
...
instance Command BudgetCommand where
    runCmd BudgetStatus = runWithBudget $
        \b -> do
...
main = getProgOpts >>=
    \ops -> do
            ...
            printEAndExit
  where
    initState o = CliState { mOpts = mainOpts o }

getAccount :: (MonadThrow m) => Name -> BudgetAccounts -> m Account
getAccount name = maybe (throwM $ AccountNotExistsException name) return .
    DM.lookup name
