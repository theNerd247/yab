...
newtype TransEntry = TransEntry { getTransEntry :: Entry }
...
data SortedEntries = SortedEntries { sortedEntries    :: EntriesMap
...
mapTransactions es dmap =
    SortedEntries { sortedEntries = DM.mapKeys fromJust .
                      DM.filterWithKey (\k _ -> isJust k) $
                      entriesSorted
                  , nonSortedEntries = mconcat .
                      fmap snd .
                          DM.toList . DM.filterWithKey (\k _ -> isNothing k) $
                      entriesSorted
                  }
  where
    entriesSorted = foldr mkmap
                          DM.empty
                          [ (getName e dmap, [ e ])
                          | e <- es ]
    getName (Entry{entryDesc = d}) dmap =
        getTransEntryName d dmap
    mkmap (k, a) m = DM.insertWith (++) k a m

getTransEntryName :: String -> DescMap -> Maybe Name
getTransEntryName d = fst .
    DM.foldrWithKey getMaxKey (Nothing, 0) . fmap (getWordCount d)
  where
    getMaxKey k v (kk, m)
        | v > m = (Just k, v)
        | otherwise = (kk, m)
...
getWordCount d = sum . fmap getKeywordCount
  where
    getKeywordCount keyword =
        DB.bool 0 1 $ DL.isInfixOf keyword d
...
updateEntries entryMap bas =
    DM.mapWithKey (\k a -> unionEntries a (DM.lookup k entryMap)) bas
  where
    unionEntries a Nothing =
        a
    unionEntries a (Just e) =
        a { accountEntries = DL.union (accountEntries a) e }
