...
instance Serialize (DM.Map Name Account) where
    serialize fpath m = sequence_ [ serialize (makeAccountPath fpath n) a
                                  | (n, a) <- DM.toList m ]
...
instance Serialize Budget where
    serialize fpath b = do
        ...
        serialize (getBudgetDirPath fpath)
                  (budgetAccounts b)
    deserialize fpath = do
        ...
        cas <- sequence $
                   deserializeAccount (getBudgetDirPath fpath) <$> DM.keys (budgetAccounts b)
        -- merge the Account data
        return $
            b { budgetAccounts = DM.unionWith mergeAccountData
                                              (budgetAccounts b)
                                              (DM.fromList cas)
              }
...
makeAccountPath basePath name =
    basePath </> "accounts" </> name <.> "csv"

deserializeAccount bp n =
    deserialize (makeAccountPath bp n) >>= return . ((n,))